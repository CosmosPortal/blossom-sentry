...
const select = new SelectMenu({
	id: /^(EditAvatarBannerExtension_|EditAvatarBannerSize_).+$/,
...
execute(select, async (interaction) => {
	...
	const extension = customId === "EditAvatarBannerExtension" ? interaction.values[0] : (selectedExtension as AvatarExtension);
	const size = Number(customId === "EditAvatarBannerSize" ? interaction.values[0] : selectedSize) as AvatarSize;
	...
	const embed = new EmbedBuilder().setTitle(`@${user.tag}'s Banner`).setDescription(`- **Extension** • ${extension}\n- **Size** • ${size}`).setImage(bannerURL).setColor(Utility.DefaultColor());
	...
	const actionRowTwo = new StringSelectMenuBuilder({
		custom_id: `EditAvatarBannerExtension_${user.id}_${extension}_${size}`,
		select_options: [
	...
	const actionRowThree = new StringSelectMenuBuilder({
		custom_id: `EditAvatarBannerSize_${user.id}_${extension}_${size}`,
		select_options: [
