...
const select = new SelectMenu({
	id: /^(EditAvatarGuildExtension_|EditAvatarGuildSize_).+$/,
...
execute(select, async (interaction) => {
	...
	const extension = customId === "EditAvatarGuildExtension" ? interaction.values[0] : (selectedExtension as AvatarExtension);
	const size = Number(customId === "EditAvatarGuildSize" ? interaction.values[0] : selectedSize) as AvatarSize;
	...
	const embed = new EmbedBuilder().setTitle(`@${member.user.tag}'s Server Avatar`).setDescription(`- **Extension** • ${extension}\n- **Size** • ${size}`).setImage(avatarURL).setColor(Utility.DefaultColor());
