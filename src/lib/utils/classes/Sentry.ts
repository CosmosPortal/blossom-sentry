...
export class Sentry {
	...
	/**
	 * Checks if the user has agreed to the client's Terms of Service and Privacy Policy
	 * @param {Snowflake} userId - The user's ID to check
	 * @returns {Promise<boolean>} The user's agreement status
	 */
	public static async MemberAgreementStatus(userId: Snowflake): Promise<boolean> {
		const { MemberAgreement } = await FindOrCreateEntity(AccountSecurity, { Snowflake: userId });
